function [jerk_vec] = fJerkFunction(d, theta, paramVector)
    ...
    thetaDotFunction = (sqrt(paramVector.mju)./r.^2) ./ sqrt((1./r + 2.*c + 6.*d.*theta(1,:) + 12.*e.*theta(2,:) + 20.*f.*theta(3,:) + 30.*g.*theta(4,:)));
    thrustFunction = -paramVector.mju ./ (2 .* r.^3 .* cos(gamma)) .* (6.*d + 24.*e.*theta(1,:) + 60.*f.*theta(2,:) + 120.*g.*theta(3,:) - tan(gamma)./r) ./ (1./r + 2.*c + 6.*d.*theta(1,:) + 12.*e.*theta(2,:) + 20.*f.*theta(3,:) + 30.*g.*theta(4,:)).^2;
